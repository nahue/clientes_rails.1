class MetronicNavigation < SimpleNavigation::Renderer::Base
  ...
  def li_icon(item)
    classes = item.selected? ? "active #{item.html_options[:opts][:class]}" : item.html_options[:opts][:class]
    #classes = item.selected? ? 'active' : nil
    content_tag(:li, content_tag(:a, content_tag(:i, nil, :class => item.html_options[:opts][:icon]) + content_tag(:span, item.name, :class => "title"), {:href => item.url, :method => item.method}.merge(item.html_options.except(:class,:id,:opts))), { class: classes })
  end
end